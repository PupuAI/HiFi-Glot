...
class SourceFilterFormantSynthesisGenerator(torch.nn.Module):
    ...
    def __init__(self, fm_config, g_config, pretrained_fm:str = None, freeze_fm:bool = True):
        """
        fm_config: Config object for feature mapping model:
        g_config: Config object for HiFi generator.
        """
    ...
    def forward(self, input_features, f0):
        x = self.feature_mapping(input_features)

        x_allpole, x_mel = torch.split(x, (self.allpole_order+1, self.num_mels), dim=1)
        ...
        gan_input = torch.cat([x_mel, x_rcoef, x_gain], dim=1)
        excitation = self.hifi_generator(gan_input, f0)
