...
class ConvolutionLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.use_conditioning:
            self.cond_1x1 = torch.nn.Conv1d(
                cond_channels,
                self.channel_mul * residual_channels,
                kernel_size=1,
                bias=True,
                device=device,
                dtype=dtype,
    ...
    def _parse_activation(self, activation):
        activation_fun, channel_mul = ConvolutionLayer.activations.get(
    ...
    def forward(self, input, cond_input=None, sequential=False):
        ...
        if cond_input is not None and not self.use_conditioning:
            raise RuntimeError(
                "Module has not been initialized to use conditioning, \
    ...
    def _forward_native(self, input, cond_input):
        ...
        if self.channel_mul == 2:
            ...
            x = self.activation_fun[0](x[:, :R, :]) * self.activation_fun[1](
                x[:, R:, :]
            )
        else:
