...
def _gated_activation(x: torch.Tensor) -> torch.Tensor:
    ...
    assert (
        x.size(1) % 2 == 0
    ), f"Input must have an even number of channels, shape was {x.shape}"
    half = x.size(1) // 2
...
class ActivationFunction(torch.autograd.Function):
    ...
    @staticmethod
    def forward(
        ctx, input: torch.Tensor, activation: str, time_major: bool = True
    ) -> torch.Tensor:
    ...
    def backward(ctx, d_output):
        raise NotImplementedError(
            "Backward function not implemented for sequential processing"
        )
