    ...
    def forward(
        ...
        if cond_input is not None:
            if self.use_film and cond_input.size(1) != 2 * self.out_channels:
                raise ValueError(
                    f"Cond input number of channels mismatch."
            ...
            if cond_input.size(2) != input.size(2):
                raise ValueError(
                    f"Mismatching timesteps, "
                    f"input has {input.size(2)}, cond_input has {cond_input.size(2)}"
                )

        return self._forward_native(
            input=input, cond_input=cond_input, causal=self.causal
        )

    def _forward_native(
        self, input: torch.Tensor, cond_input: torch.Tensor, causal: bool = True
    ) -> torch.Tensor:
