...
class WaveNet(torch.nn.Module):
    ...
    def receptive_field(self):
        return (
            self.input.receptive_field
            + self.stack.receptive_field
            + self.output1.receptive_field
            + self.output2.receptive_field
        )
    ...
    def forward(self, input, cond_input=None):
        ...
        if cond_input is not None and not self.use_conditioning:
            raise RuntimeError(
                "Module has not been initialized to use conditioning, but conditioning input was provided at forward pass"
            )
        ...
        _, skips = self.stack(
            x, cond_input
        )  # TODO self.stack must be called something different, torch.stack is different
