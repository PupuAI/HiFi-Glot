...
class BiquadBaseFunctional(torch.nn.Module):
    ...
    def _params_to_direct_form(
        self, freq: torch.Tensor, gain: torch.Tensor, Q: torch.Tensor
    ...
    def forward(
        ...
        batch, out_channels, in_channels, n_frames = self._check_param_sizes(
...
class BiquadPeakFunctional(BiquadBaseFunctional):
    ...
    def _params_to_direct_form(
        ...
        if torch.any(freq > 1.0):
            raise ValueError(
                f"Normalized frequency must be below 1.0, max was {freq.max()}"
            )
        if torch.any(freq < 0.0):
            raise ValueError(
                f"Normalized frequency must be above 0.0, min was {freq.min()}"
            )
...
class BiquadModule(torch.nn.Module):
    ...
    def set_freq(self, freq):
        ...
        if freq.min() < 0.0:
            raise ValueError("Maximum normalized frequency is smaller than 0.0.")
    ...
    def get_frequency_response(
        self, n_timesteps: int = 2048, n_fft: int = 2048
    ) -> torch.Tensor:
...
class BiquadParallelBankModule(torch.nn.Module):
    ...
    def __init__(
        ...
        self.fs = fs
        self.filter_bank = BiquadModule(
            in_channels=num_filters, out_channels=1, fs=fs, func=func
        )
