...
class LinearPredictor(torch.nn.Module):
    ...
    def __init__(
        self, n_fft: int = 512, hop_length: int = 256, win_length=512, order=10
    ):
        """
        Args:
            n_fft (int): FFT size
    ...
    def synthesis_filter(self, e: torch.Tensor, a: torch.Tensor) -> torch.Tensor:
        """
        Args:
    ...
    def prediction(self, x: torch.Tensor, a: torch.Tensor) -> torch.Tensor:
        """
        Args:
