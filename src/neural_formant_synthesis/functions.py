...
def spectral_centroid(x, sr):
    ...
    centroid = torch.sum(mag_spec*freqs, dim = -1) / (torch.sum(mag_spec, dim = -1) + 1e-10)
    return centroid
...
def frame_energy(x):
    ...
    energy = torch.sum(torch.square(torch.abs(x)), dim = -1) + 1e-9
...
def pitch_extraction(x, sr, window_samples, step_samples, fmin = 50, fmax = 500):
    ...
    time_step = float(step_samples) / sr
    l_pad = int(np.ceil(1.5 / fmin * sr))
    r_pad = step_samples * ((len(audio) - 1) // step_samples + 1) - len(audio) + l_pad + 1
    audio = np.pad(audio, (l_pad, r_pad))
    s = parselmouth.Sound(audio, sampling_frequency=sr).to_pitch_ac(
        time_step=time_step, voicing_threshold=0.6,
        pitch_floor=fmin, pitch_ceiling=fmax)
    pitch = s.selected_array['frequency'].astype(np.float32)
    
    s = parselmouth.Sound(audio, sampling_frequency=sr).to_formant_burg(
        time_step=time_step,
        window_length=time_step*4,
    )
    formants = []
    for i in range(0, pitch.shape[-1]):
        t = i * time_step
        f1 = s.get_value_at_time(1, t)
        f2 = s.get_value_at_time(2, t)
        f3 = s.get_value_at_time(3, t)
        formants.append(np.array([f1, f2, f3]))

    # Interpolate unvoiced tokens
    pitch, voicing_flag = interpolate_unvoiced(pitch)
    
    formants = np.stack(formants)
    try:
        for i in range(0, 3):
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                formants[..., i] = np.log2(formants[..., i])
            
            formants[..., i][np.isnan(formants[..., i])] = np.interp(
                np.where(np.isnan(formants[..., i]))[0],
                np.where(~np.isnan(formants[..., i]))[0],
                formants[..., i][~np.isnan(formants[..., i])])
            
            formants[..., i] = 2 ** formants[..., i]
    except:
        pass

    # Convert to torch
    return torch.from_numpy(formants), torch.from_numpy(pitch)[None], torch.tensor(voicing_flag, dtype = torch.int)